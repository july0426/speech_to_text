...
def get_data_from_mysql():
    ...
    cocur = db.cursor()
    sql = 'select id,save_path,cc,tel_number,`size`,file_name from show_caller_audio where speech_status = 0 and audio_language="en" and cc=966 ORDER BY id DESC limit 1'
    try:
        cocur.execute(sql)
        # 获取一条数据
        data = cocur.fetchone()
...
class WSInterfaceProtocol(WebSocketClientProtocol):
    ...
    def onMessage(self, payload, isBinary):
        ...
        if isBinary:
            ...
            # if in streaming
            elif 'speaker_labels' in jsonObject:
                jsonObject = json.loads(payload.decode('utf8'))
                speaker_labels = jsonObject['speaker_labels']
                def delConfidence(speaker_labels):
                    for i in speaker_labels:
                        i.pop('final')
                        i.pop('to')
                # delConfidence = lambda x: for i in x i.pop('confidence')
                # delFinal = lambda x:i.pop('final') for i in x
                delConfidence(speaker_labels)
                print(speaker_labels)
                self.summary[self.uttNumber]['speaker_labels'] += speaker_labels
                ...
                # regular hypothesis
                    ...
                    if bFinal:
                        ...
                        self.summary[self.uttNumber]['timestamps'] += timestamps
                    else:
...
if __name__ == '__main__':
    ...
    def speech_to_text(audio, proxy):
        ...
        if os.path.exists(temp_file_path):
            ...
            if pipe == 0:
                ...
                return False
        try:
            text_dict = get_text(temp_file_path, proxy)
            db = MySQLdb.connect('163.172.254.11', 'speech_text_ar', 'speechIBM', 'show_caller_audio',
                                 use_unicode=True,
                                 charset='utf8')
            cocur = db.cursor()
            if text_dict['text']:
                text = text_dict['text'].replace('%HESITATION','hmm~')
                text_processed = text_dict['text_processed'].replace('%HESITATION','hmm~')
                ...
                try:
                    ...
                    print('save to mysql faild:reason is    :')
                    print(str(e))
                    db.rollback()
                # 吧text存入数据库
                sql = "insert into show_caller_audio_text(cc,tel_number,save_path,audio_language,`size`,speech_text,processed_text,file_name) VALUES ('%s','%s','%s','en','%s','%s','%s','%s')" % (
                    audio['cc'], audio['tel_number'], audio['save_path'], audio['size'], text,text_processed,
                    audio['file_name'])
            else:
                # 更新audio数据表为识别失败-10
    ...
    log_file = open("/home/wwwroot/show_caller_audio/message_test.log", "w")
