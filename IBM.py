    ...
    try:
        ...
        if data:
            ...
            if duration is not False:
                return {'id': id, 'save_path': data[1], 'duration': duration}
            else:
                return False
        else:
...
def get_account(audio_duration):
    ...
    db = MySQLdb.connect('localhost', 'root', '123456', 'test')
    cocur = db.cursor()
    sql = 'select id,username,password,duration from show_caller_IBM_account where status = 0 limit 1'
    try:
        cocur.execute(sql)
        # 获取一条数据
        data = cocur.fetchone()
        if data:
            ...
            if new_duration <60000:
                ...
                #     cocur.execute(sql)
                #     db.commit()
                #     print('IBM_account duration update success!')
                # except Exception as e:
                #     print(str(e))
                #     db.rollback()
                #     print('IBM_account duration update faild....')
                ...
                sql = 'update show_caller_IBM_account set status=%s where id=%s' % (int(time.time()), id)
                try:
                    cocur.execute(sql)
                    db.commit()
                    print('IBM_account status update success!')
                except Exception as e:
                    print(str(e))
                    db.rollback()
                    print('IBM_account status update faild....')
                # 如果超出,就重新选择一个账号(递归)
...
def get_proxy():
    ...
    cursor = db.cursor()
    # 从数据库中取出代理
    sql = "select id,proxy from proxy order by status asc limit 1"
    try:
        self.cursor.execute(sql)
        proxies = self.cursor.fetchone()
        if proxies:
            # 更新代理时间戳
            sql = "update proxy set status=%d where id=%d" % (int(time.time()), proxies[0])
            cursor.execute(sql)
            proxie = proxies[1]
            proxy = {
                'http': 'http://%s' % proxie,
    ...
    try:
        ...
        if text:
            ...
            try:
                cocur.execute(sql)
                db.commit()
                print('IBM_duration update success!')
            except Exception as e:
                print('IBM_duration update faild.....')
                print(str(e))
                db.rollback()
            #   正则匹配出识别后的音频
            ...
            print(text)
            # 吧text存入数据库
            sql = 'update show_caller_audio set text="%s",speech_status="%s" where id =%s' % (text,int(time.time()),audio['id'])
            try:
                cocur.execute(sql)
                db.commit()
                print('Text saved sucessed!')
            except Exception as e:
                print(str(e))
                print('Text save faild....')
                db.rollback()
        else:
            # 更新audio数据表为识别失败-10
