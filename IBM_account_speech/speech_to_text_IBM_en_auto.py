...
def speech_to_text(account,audio,proxy):
    ...
    try:
        with open(join(dirname(__file__), temp_file_path),
                  'rb') as audio_file:
            text = json.dumps(speech_to_text.recognize(audio_file, content_type='audio/mp3',model='en-US_BroadbandModel', timestamps=True,word_confidence=False,proxies=proxy),indent=2)
        ...
        if text:
            ...
            if text:
                ...
                print(text)
                # 吧text存入数据库
                sql = 'insert into show_caller_audio_text(cc,tel_number,save_path,audio_language,`size`,speech_text,file_name) VALUES ("%s","%s","%s","en","%s","%s","%s")' % (audio['cc'],audio['tel_number'],audio['save_path'],audio['size'],text,audio['file_name'])
            else:
                # 更新audio数据表为识别失败-10
