...
def get_data_from_mysql():
    ...
    cocur = db.cursor()
    sql = 'select id,save_path,cc,tel_number,`size`,file_name from show_caller_audio where speech_status = 0 and audio_language="ar" and cc=964 ORDER BY id DESC limit 1'
    try:
        cocur.execute(sql)
        # 获取一条数据
        data = cocur.fetchone()
