...
def get_data_from_mysql():
    ...
    cocur = db.cursor()
    sql = 'select id,save_path,cc,tel_number,`size` from show_caller_audio where speech_status = 0 and audio_language="ar" and cc=964 limit 1'
    try:
        cocur.execute(sql)
        # 获取一条数据
        data = cocur.fetchone()
        if data:
            ...
            if duration is not False:
                return {'id': id, 'save_path': data[1], 'duration': duration,'cc':data[2],'tel_number':data[3],'size':data[4]}
            else:
                return False
        else:
    ...
    try:
        ...
        if text:
            ...
            if text:
                ...
                print(text)
                # 吧text存入数据库
                sql = 'insert into show_caller_audio_text(cc,tel_number,save_path,audio_language,`size`,speech_text) VALUES ("%s","%s","%s","ar","%s","%s")' % (audio['cc'],audio['tel_number'],audio['save_path'],audio['size'],text)
            else:
                # 更新audio数据表为识别失败-10
